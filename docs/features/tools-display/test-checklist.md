...
**Checklist:** `docs/features/tools-display/test-checklist.md`  
...
| --- | --- | --- | --- |
| 1 | `display_glob` / `display-glob-basic` | Glob card + paths — **no JSON expander** | ✓ |
| 2 | `display_grep` / `display-grep-basic` | Grep card + matches | ✓ |
| 3 | `display_read` / `display-read-readme` | Read card + content preview | ✓ |
