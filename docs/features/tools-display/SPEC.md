...
**Related:** [`docs/PRD.md`](../../PRD.md) FR-C3, FR-C6, FR-C7, FR-S3, FR-S6; roadmap “Tools display and user interaction”  
...
| Context | Allowed? |
| --- | --- |
| Default transcript / approval UX | **No** expanders for tool payload or results |
| Debug mode | Optional collapsed expander labeled **Raw tool payload** |
| Structured output / unrelated UI | Out of scope (may still use expanders) |

### 3.2 Layout

- Tool cards use a **bordered container** (or equivalent bordered block), never a collapsed `st.expander` as the primary surface.
- Header line: `**{FamilyLabel}** · {StatusLabel}` plus optional short meta (path, row count, pattern).
- Status labels: `Running` | `Completed` | `Failed` (map from `running` / `completed` / `error`).
- While a tool is `running`, show a short progress caption (e.g. “Searching content…”, “Reading…”).
...
7. Manual checklist under `docs/features/tools-display/test-checklist.md` passes on a live session.
