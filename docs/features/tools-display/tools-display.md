...
Transcript tool calls render as meaningful bordered cards (Glob, Grep, Read, Write, Edit, Bash, SQL, AskUserQuestion, ExitPlanMode, generic). Interactive HITL covers approvals, AskUserQuestion (radio / Other… / Submit), and ExitPlanMode (Approve plan / Reject). Raw JSON payloads appear only in CoCo debug mode.
