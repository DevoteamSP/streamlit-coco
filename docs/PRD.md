...
**Author:** —  
**Status:** Alpha `0.1.0` (product decisions locked; docs synced to code)  
...
| UC4 | **Multi-step wizard** — app drives turns programmatically between Streamlit steps | `CocoSession.send()` (headless `stream()` still TBD) |
...
- Typed exception hierarchy still TBD (SDK errors currently surface as runtime failures).
...
- Methods: `send(prompt)`, `cancel()`, `close()`, `reset()`, `sync_options()`.
- Properties: `is_running`, `needs_polling`, `messages` / transcript, `last_result`, `status`.
- **`stream()` async generator** — not implemented yet (`NotImplementedError`); UI uses worker + polling instead of awaiting `stream()` in app code.
...
| FR-C6 | Permission card: Approve / Deny / Always allow | ✅ (no Edit/Write diff preview yet) |
| FR-C7 | Generic user input request surface | ❌ deferred (AskUserQuestion shipped; CCv2 `provide_input` dropped) |
...
| `coco.request_input(...)` | Clarification helper | ⚠️ thin `st.text_input` stub |
...
- **`st_coco.request_input(question, schema=None, key=...)`** — currently a thin `st.text_input` stub; full component sub-mode TBD.
- Hook integration: `UserPromptSubmit` / custom `on_input_required` — not wired yet (CCv2 `provide_input` trigger was dropped; use AskUserQuestion).

#### 6.3.3 Plan mode

When `permission_mode="plan"`, the legacy CCv2 UI shows a planning banner. **Execute plan** CTA is not implemented yet.
...
| --- | --- | --- |
| FR-S1 | Assistant text appends in place without flicker (stable DOM keys per message id) | P0 |
| FR-S2 | Auto-scroll to bottom while running; pause auto-scroll if user scrolls up | P1 |
| FR-S3 | Tool cards collapse by default except errors | P1 |
| FR-S4 | Stop button sends SDK cancel/interrupt; UI marks run as `cancelled` | P0 |
| FR-S5 | Show spinner / pulsing indicator during tool execution | P1 |
| FR-S6 | Render code blocks and SQL with syntax highlighting | P1 |
| FR-S7 | Copy-to-clipboard on assistant messages and tool results | P2 |
...
| ID | Requirement |
| --- | --- |
| FR-ST1 | Component `key` maps to `st.session_state[key]` subtree: `{ transcript, pending_approval, status, options_hash }` |
| FR-ST2 | Changing `options` mid-session calls `session.reset()` or warns user (configurable) |
| FR-ST3 | `reset()` clears transcript and closes SDK client |
| FR-ST4 | Transcript persists across reruns; optional `max_messages` truncation with “load earlier” |
...
| **Packaging** | Wheel includes static frontend assets; `pip install streamlit-coco[sdk]`. PyPI publish pending. |
...
| Docker / K8s | ✅ | Bake CLI + creds into image/secrets |
...
- [ ] CI workflow
- [ ] Recorded NDJSON fixture corpus
...
- [ ] `CocoSession.stream()` async API

### Phase 2 — Component / UI MVP

- [x] CCv2 component registration + **static** frontend assets (Vite build deferred)
...
- [ ] Plan mode **Execute** CTA
- [ ] Full `request_input` / `provide_input` clarification UX
- [ ] Edit/Write diff preview on approvals
...
**Current:** Alpha `0.1.0` locally; Phase 4 release items remain.
...
| CoCo CLI not on Streamlit Cloud | Blocks hosted demos | Document Docker/SPCS/local deployment; no remote proxy planned |
...
| 14.7 | Frontend build? | **Static** html/css/js under `streamlit_coco/frontend/` for now; Vite/`asset_dir` packaging deferred. |
